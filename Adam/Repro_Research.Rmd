...
# changes the 'environment' the functions use to render the file, so that we can use the data.table package
use.data.table()

		renderMultiDocument(
				rmd_input = 'Baseline_SGP_Report.Rmd',
...
##  Look at the files and directories produced

### `cache`

When we render the file, we produced a directory called the "Baseline_SGP_Report_cache," which saves some interim results from running the R code.  This is useful when drafting the document and changing `R` code because we don't have to re-run time consuming calculations, data structuring, etc.  However, we don't want this cache to be included in our Github repository.  We will "ignore" it using the .gitignore file.
...
If we have either produced figures or plots to include in the document in R, or just have image files we want to include in the document, they need to be placed in the img/ directory.  In this case we created the Goodness of Fit plots and then moved those into the img/ directory using R code.
...
When we render the file, the first output format we produced was a standalone HTML file.  Standalone means that it has the CSS, JS and .png images embedded in code within the document.  That potentially makes it  very large, but makes it easily transportable (you can simply give the file to someone and they can open it in a browser without needing a complex file system backing it).
...
If we render the file in other possible formats, we will also produce file directories with these format files (and potentially other interim documents, such as .md and .tex, if we supply the argument `cleanup_aux_files=TRUE`.  
